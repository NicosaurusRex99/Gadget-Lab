...
public class RenderProjectile<T extends ThrowableProjectile> extends EntityRenderer<T> {
    protected final ResourceLocation TEXTURE;
    protected final RenderType renderType;

    public RenderProjectile(final Context context, final String name) {
        this(context, new ResourceLocation(Main.MODID, "textures/projectiles/" + name + ".png"));
    }
    public RenderProjectile(final Context context) {
        this(context, new ResourceLocation("textures/particle/generic_0.png"));
    }
    public RenderProjectile(final Context context, final ResourceLocation texture) {
        super(context);
    ...
    private static void pos(VertexConsumer vertexBuilder, Matrix4f matrix4f, Matrix3f normal, int lightmapUV, float x, float y, float u, float v) {
        vertexBuilder.vertex(matrix4f, x - 0.5F, y - 0.25f, 0).color(255, 255, 255, 255).uv(u, v).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmapUV)
                //TODO - pose
//                .normal(normal, 0, 1, 0)
                .endVertex();
    }
